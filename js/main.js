...
function setItems(products) {
  ...
  if (items != null) {
    ...
    //if clicked on product for first time, set inCart to 1
    products.inCart = 1;
    items = {
      [products.name]: products,
    };
    ...
    Object.values(items).map((products) => {
      productContainer.innerHTML += `<div class="products">${
        products.name
            ...
            <div class"price">Item price: R${products.price},00</div>
            <div class="quantity">Quantity: ${products.inCart}</div>
...
$("#confirm-order").click(function () {
  ...
  alert(
    "Success! You have just bought some sweet products! \n Your ref number is: " +
      ref
  );
});

// on homepage, the Deelz logo in the body has the following animation
$(".homepagelogo").fadeOut(3000).fadeIn("slow");
...
$(".pageHeader").ready(function () {
  $(".pageHeader").fadeOut(3000).fadeIn("slow");
});
